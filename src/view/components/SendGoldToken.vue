    ...
    <v-dialog v-model="utils.show_modal" width="620">
        <v-card class="pb-2">
            ...
            <div class="m-5" v-if="utils.transaction_active" ref="formContent">
                ...
                <v-text-field @input="onInputAmount()" prefix="cGLD" solo v-model.number="$v.swap.amount.$model"></v-text-field>
      ...
      methods: {
         ...
         sendTransaction() {
            if(this.swap.amount <= this.gold.balance) {
               ...
               setTimeout(() => {
                  this.$store.dispatch(SEND_GOLD_TOKEN, { amount: this.swap.amount, receiver: this.swap.receiver }).then((res) => {
                     console.log(res);
